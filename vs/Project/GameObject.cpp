...
void Engine::GameObject::Update(float deltaTime)
{
	...
	if (Engine::GameObjectState::SPAWN == state && y <= 60) {
...
bool Engine::GameObject::collidesWith(Engine::Cat_Player* player) {
	return sprite->GetBoundingBox()->CollideWith(player->getBoundingBox());
}

void Engine::GameObject::setState(GameObjectState current) {
	this->state = current;
}